			...
			<View style={{ flexGrow: 1, backgroundColor: MainBackground, }}>
				{(gstore.me!.role === 'admin' && this.props.mode !== 'mine') ? (
					<View style={{
						...
						<TextInput
							value={this.search}
							onChangeText={e => { this.search = e; this.onLoad(); }}
				...
				{this.loading ? <View style={{ flexGrow: 1, }}><ActivityIndicator color={MainLight} style={{ marginTop: 40 }} size="large" /></View> : (
					<ScrollView style={{
						paddingLeft: '6%',
						paddingRight: '6%',
						marginTop: 0,
						...
						{ods.length ? (
							...
							<View style={{ alignItems: 'center', justifyContent: 'center', marginTop: 50, width: '90%', alignSelf: 'center' }}>
						...
						{gstore.me!.role === 'user' ? (
							<View style={{ width: '100%', justifyContent: 'center', alignItems: 'center', marginTop: 40, marginBottom: 20 }}>
								<FButton onPress={() => {
									//@ts-ignore
