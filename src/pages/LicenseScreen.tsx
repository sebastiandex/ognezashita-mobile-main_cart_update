...
class LicenseScreen extends PureComponent {
	...
	render() {
		...
		return (
			<ScrollView style={{ flexGrow: 1, }} contentContainerStyle={{ alignItems: 'stretch', justifyContent: 'flex-start' }}>
				<View style={{ alignItems: 'center', justifyContent: 'center', marginTop: 50, marginBottom: 50, width: '70%', alignSelf: 'center' }}>
					<Text style={{ textAlign: 'center', fontSize: 16, color: '#404040', lineHeight: 24 }}>Вы можете ознакомиться с нашими действующими лицензиями на проведение всех регламентных работ</Text>
				</View>
				<View style={{ paddingHorizontal: 20, flexDirection: 'row', justifyContent: 'space-around', alignItems: 'flex-start' }}>
					{gstore.licenses.map((l, idx) => (
						<View key={l.id} style={{ alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexGrow: 0, flexShrink: 1, flexBasis: (width - 100) / 2, height: height * 0.28, borderRadius: 10, borderWidth: 1, borderColor: '#e0e0e0', backgroundColor: 'white' }}>
